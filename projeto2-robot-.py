...
from pf import Particle, create_particles, move_relative
import numpy as np
...
# Robo
robot = Particle(largura/2, altura/2, math.pi/4, 1)
...
def cria_particulas(minx=0, miny=0, maxx=largura, maxy=altura, n_particulas=num_particulas):
		...
		"""   
		return create_particles(robot.pose(),var_x = 350, var_y = 350, var_theta = math.pi/2, n_particulas)
...
def move_particulas(particulas, movimento):
		...
		dl= 5
		da= math.pi/20
		movimento[0]= np.random.normal(movimento[0],dl)
		movimento[1] =np.random.normal(movimento[1],da)
		
		
		for p in particulas:
				move_relative(p, movimento)
